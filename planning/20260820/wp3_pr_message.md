...
3. **Added columnar cell-cell derivation from synapse table** —
   `write_utils.py::derive_cell_cell_connectivity` groups Polars synapse table rows
  by project and pre- and post-synaptic cells, always emits `SYNAPSE_COUNT`,
  and optionally emits `SUM_ANATOMICAL_SIZE` with an explicit source column
  and unit. IDs are readable strings derived from connectome, pre- and
  post-synaptic cells, and measurement type; project scope and optional
  synapse-table provenance do not affect the generated ID.
...
4. **Consolidated cell-cell ETL storage** —
   ...
   uses the derivation API, and writes the derived context to the same canonical
   table; `etl_example_prompt.md` was updated accordingly.
...
replacement of those raw canonical writes.
...
- `write_utils.py::_cell_cell_measurement_id`: derived IDs use the readable
  `connectome_pre_post_measurement-type` form; project scope and source
  provenance are deliberately excluded. This private helper does not yet
...
- `write_utils.py::derive_cell_cell_connectivity`: size aggregation rejects any
  null size rather than reporting a partial total.
...
- Minnie and V1DD notebook writes use canonical paths but still call raw Delta
  APIs; review their `(project_id, connectome_id)` overwrite predicates.
